...
function list(data2, decode, pushPath, index3, emptyList) {
...
function accept(values2) {
  return attribute2("accept", join(values2, ","));
}
...
class EventFired extends CustomType {
  constructor(kind, path, name2, event) {
    super();
    this.kind = kind;
    this.path = path;
    this.name = name2;
...
function do_add_event(handlers, path, name2, handler) {
  return insert2(handlers, event(path, name2), handler);
}
...
function decode2(cache, path, name2, event2) {
...
function handle(cache, path, name2, event2) {
  let _pipe = decode2(cache, path, name2, event2);
  return ((_capture) => {
  ...
  #createAttribute(node, attribute3) {
    ...
    switch (kind) {
      ...
      case event_kind: {
        ...
        handlers.set(name2, (event2) => this.#handleEvent(attribute3, event2));
        break;
      }
    }
  }
...
class Runtime {
  constructor(root2, [model, effects], view, update2, options) {
    ...
    const decodeEvent = (event2, path, name2) => decode2(this.#cache, path, name2, event2);
...
class Runtime2 {
  ...
  send(message) {
    if (Message$isClientDispatchedMessage(message)) {
      ...
      const { name: name2, data: data2 } = message;
  ...
  #handle_client_message(message) {
    if (ServerMessage$isBatch(message)) {
      ...
      const { path, name: name2, event: event2 } = message2;
      const [cache, result] = handle(this.#cache, path, name2, event2);
      this.#cache = cache;
      if (!Result$isOk(result)) {
        return this.#vdom;
      }
...
// build/dev/javascript/shared/shared/shared_thumbnail.mjs
class Thumbnail extends CustomType {
  constructor(public_id, asset_id, description, creator, premium, private$, user_liked, show_on_profile) {
    super();
...
function thumbnail_decoder() {
  return field("public_id", string2, (public_id) => {
    return field("asset_id", string2, (asset_id) => {
      return field("description", optional(string2), (description) => {
        return field("creator", string2, (creator) => {
          return field("premium", bool, (premium) => {
            return field("private", bool, (private$) => {
              return field("user_liked", bool, (user_liked) => {
                return field("show_on_profile", bool, (show_on_profile) => {
                  return success(new Thumbnail(public_id, asset_id, description, creator, premium, private$, user_liked, show_on_profile));
                });
              });
            });
          });
...
// build/dev/javascript/shared/shared/shared_stats.mjs
class Stats extends CustomType {
  constructor(views, likes, downloads) {
    super();
    this.views = views;
    this.likes = likes;
...
function stats_decoder() {
  return field("views", int2, (views) => {
    return field("likes", int2, (likes) => {
      return field("downloads", int2, (downloads) => {
        return success(new Stats(views, likes, downloads));
      });
    });
  });
}

// build/dev/javascript/shared/shared/shared_photo.mjs
class Photo extends CustomType {
  constructor(thumbnail, stats, title, location, camera, created_at, tags) {
    super();
...
function photo_decoder() {
  return field("thumbnail", thumbnail_decoder(), (thumbnail) => {
    return field("stats", stats_decoder(), (stats) => {
      return field("title", optional(string2), (title) => {
        return field("location", optional(string2), (location) => {
          return field("camera", optional(string2), (camera) => {
            return field("created_at", float2, (created_at) => {
              return field("tags", list2(string2), (tags) => {
                return success(new Photo(thumbnail, stats, title, location, camera, created_at, tags));
              });
            });
          });
        });
...
// build/dev/javascript/shared/shared/shared_user.mjs
class User extends CustomType {
  constructor(username, first_name, last_name, bio, available_for_hire, premium, photos) {
    super();
...
function user_decoder() {
  return field("username", string2, (username) => {
    return field("first_name", string2, (first_name) => {
      return field("last_name", optional(string2), (last_name) => {
        return field("bio", optional(string2), (bio) => {
          return field("available_for_hire", bool, (available_for_hire) => {
            return field("premium", bool, (premium) => {
              return field("photos", list2(thumbnail_decoder()), (photos) => {
                return success(new User(username, first_name, last_name, bio, available_for_hire, premium, photos));
              });
            });
          });
        });
...
class PhotoById extends CustomType {
  constructor(id2) {
    super();
    this.id = id2;
  }
}
class CollectionById extends CustomType {
  constructor(id2) {
    super();
    this.id = id2;
  }
}
class UserByName extends CustomType {
  constructor(name2) {
    super();
    this.name = name2;
  }
...
class Join extends CustomType {
}
class Upload extends CustomType {
}
  ...
  if ($ instanceof Empty) {
    ...
    if ($1 instanceof Empty) {
      ...
      if ($2 === "") {
        ...
        let username = $2.slice(1);
        return new UserByName(username);
      } else if ($2 === "login") {
        return new Login;
      } else if ($2 === "join") {
        return new Join;
      ...
      if ($2 instanceof Empty) {
        let $3 = $.head;
        if ($3 === "photos") {
          let public_id = $1.head;
          return new PhotoById(public_id);
        } else if ($3 === "collections") {
          let collection_id = $1.head;
          return new CollectionById(collection_id);
        } else {
          return new NotFound;
        }
      } else {
...
function to_path(route) {
  if (route instanceof Index2) {
    return "/";
  } else if (route instanceof PhotoById) {
    let id2 = route.id;
    return "/photos/" + id2;
  } else if (route instanceof CollectionById) {
    let id2 = route.id;
    return "/collections/" + id2;
  } else if (route instanceof UserByName) {
    let name2 = route.name;
    return "/@" + name2;
  } else if (route instanceof Login) {
    return "/login";
  } else if (route instanceof Join) {
    return "/join";
  } else if (route instanceof Upload) {
    return "/upload";
  } else {
    return "/404";
  }
...
function href2(route) {
  return href(to_path(route));
}

// build/dev/javascript/client/layout.mjs
function nav_left(_) {
  return div(toList([class$("flex items-center gap-6")]), toList([
    a(toList([
      href2(new Index2),
      class$("text-xl font-bold tracking-tight text-primary hover:text-primary-hover transition-colors")
    ]), toList([
      span(toList([class$("text-2xl mr-1")]), toList([text2("\uD83D\uDCF8")])),
      text2("funsplash")
    ]))
  ]));
}
function nav_center() {
  return div(toList([class$("hidden md:flex flex-1 max-w-xl mx-8")]), toList([
    div(toList([
      class$("relative w-full flex items-center bg-surface-dim rounded-full border border-border-light")
    ]), toList([
      span(toList([class$("absolute left-4 text-accent text-sm")]), toList([text2("\uD83D\uDD0D")])),
      input(toList([
        type_("text"),
        placeholder("Search photos"),
        class$("w-full py-2.5 pl-11 pr-4 bg-transparent text-sm text-primary placeholder-accent rounded-full"),
        attribute2("readonly", "true")
      ]))
    ]))
  ]));
}
function logged_in_nav(username) {
  return div(toList([class$("flex items-center gap-3")]), toList([
    a(toList([
      href2(new UserByName(username)),
      class$("flex items-center gap-2 px-3 py-2 text-sm font-medium text-primary hover:bg-surface-dim rounded-lg transition-colors")
    ]), toList([
      div(toList([
        class$("w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-accent")
      ]), toList([text2("\uD83D\uDC64")])),
      span(toList([class$("hidden sm:inline")]), toList([text2(username)]))
    ])),
    a(toList([
      href("/logout"),
      class$("px-4 py-2 text-sm font-medium text-accent hover:text-primary transition-colors")
    ]), toList([text2("Log out")]))
  ]));
}
function logged_out_nav() {
  return div(toList([class$("flex items-center gap-3")]), toList([
    a(toList([
      href2(new Login),
      class$("px-4 py-2 text-sm font-medium text-accent hover:text-primary transition-colors")
...
function nav_link(label2, target, current) {
  let _block;
  let $ = isEqual(target, current);
  if ($) {
    _block = " text-primary font-semibold";
  } else {
    _block = " text-accent hover:text-primary";
  }
  let active_class = _block;
  return a(toList([
    href2(target),
    class$("hidden lg:inline-block px-3 py-2 text-sm transition-colors" + active_class)
  ]), toList([text2(label2)]));
}
function nav_right(route, auth_user) {
  return div(toList([class$("flex items-center gap-3")]), toList([
    nav_link("Explore", new Index2, route),
    a(toList([
      href2(new Upload),
      class$("hidden sm:inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-accent hover:text-primary transition-colors")
    ]), toList([
      span(toList([class$("text-base")]), toList([text2("⬆")])),
      text2("Upload")
    ])),
    div(toList([class$("hidden sm:block w-px h-8 bg-border-light")]), toList([])),
    (() => {
      if (auth_user instanceof Some) {
        let username = auth_user[0];
        return logged_in_nav(username);
      } else {
...
function navbar(route, auth_user) {
  return nav(toList([
    class$("sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-border-light")
  ]), toList([
    div(toList([
      class$("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16")
    ]), toList([nav_left(route), nav_center(), nav_right(route, auth_user)]))
  ]));
}
function footer_links(links) {
  return ul(toList([class$("space-y-2")]), map2(links, (link) => {
    let label2;
    let route;
    label2 = link[0];
    route = link[1];
...
function footer2() {
  return footer(toList([
    class$("border-t border-border-light bg-surface-dim mt-auto")
  ]), toList([
    div(toList([
      class$("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 sm:grid-cols-3 gap-8")
    ]), toList([
      div(toList([]), toList([
        h3(toList([
          class$("font-bold text-primary text-lg mb-2")
        ]), toList([text2("\uD83D\uDCF8 funsplash")])),
        p(toList([
          class$("text-sm text-accent leading-relaxed")
        ]), toList([
          text2("The internet's source of freely-usable images. Powered by creators everywhere.")
        ]))
      ])),
      div(toList([]), toList([
        h4(toList([
          class$("font-semibold text-primary text-sm mb-3")
        ]), toList([text2("Product")])),
        footer_links(toList([
          ["Explore", new Index2],
          ["Upload", new Upload]
        ]))
    ...
    div(toList([class$("border-t border-border-light")]), toList([
      div(toList([
        class$("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 text-xs text-accent")
      ]), toList([text2("© 2026 funsplash")]))
    ]))
  ]));
}

// build/dev/javascript/client/pages/auth.mjs
function auth_shell(title, subtitle, children) {
  return div(toList([
    class$("min-h-[calc(100vh-12rem)] flex items-center justify-center px-4 py-16 bg-surface-dim")
  ]), toList([
    div(toList([
      class$("w-full max-w-md glass-card rounded-2xl p-8 animate-fade-in")
    ]), prepend(div(toList([class$("text-center mb-8")]), toList([
      div(toList([class$("text-4xl mb-4")]), toList([text2("\uD83D\uDCF8")])),
      h1(toList([
        class$("text-2xl font-bold text-primary mb-2")
      ]), toList([text2(title)])),
      p(toList([class$("text-sm text-accent")]), toList([text2(subtitle)]))
    ])), children))
  ]));
}
function form_field(name2, label2, type_2, placeholder2, required) {
  return div(toList([]), toList([
    label(toList([
      for$(name2),
      class$("block text-sm font-medium text-primary mb-1.5")
    ]), toList([text2(label2)])),
    input(toList([
      type_(type_2),
      name(name2),
      ...
      class$("w-full px-4 py-3 border border-border rounded-lg text-sm text-primary placeholder-accent bg-white transition-all")
    ]))
  ]));
}
function submit_button(label2) {
  return button(toList([
    type_("submit"),
    class$("w-full py-3 px-4 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover transition-colors text-sm")
  ]), toList([text2(label2)]));
}
function alert_banner(message2, is_error) {
  let _block;
  if (is_error) {
    _block = "bg-red-50 text-red-800 border-red-200";
  } else {
    _block = "bg-green-50 text-green-800 border-green-200";
  }
  let bg_color = _block;
  return div(toList([
    class$("p-4 rounded-lg border text-sm animate-fade-in " + bg_color)
  ]), toList([text2(message2)]));
}
function login_view(error, success3) {
  let _block;
  if (error instanceof Some) {
    let err = error[0];
    _block = alert_banner(err, true);
  } else if (success3 instanceof Some) {
    let succ = success3[0];
    _block = alert_banner(succ, false);
  } else {
    _block = none2();
  }
  let alert = _block;
  return auth_shell("Welcome back", "Log in to your account", toList([
    div(toList([class$("space-y-4")]), toList([
      ...
      form(toList([
        ...
        p(toList([
          class$("text-center text-sm text-accent mt-4")
        ]), toList([
          text2("Don't have an account? "),
          a(toList([
            href2(new Join),
            class$("font-medium text-primary hover:underline")
          ]), toList([text2("Join funsplash")]))
        ]))
      ]))
    ]))
  ]));
}
function signup_view(error) {
  let _block;
  if (error instanceof Some) {
    let err = error[0];
    _block = alert_banner(err, true);
  ...
  let alert = _block;
  return auth_shell("Join funsplash", "Create your free account", toList([
    div(toList([class$("space-y-4")]), toList([
      alert,
      form(toList([
        method("POST"),
        ...
        form_field("password", "Password", "password", "At least 9 characters", true),
        form_field("first_name", "First name", "text", "Your first name", true),
        form_field("last_name", "Last name", "text", "Your last name (optional)", false),
        div(toList([]), toList([
          label(toList([
            for$("bio"),
            class$("block text-sm font-medium text-primary mb-1.5")
        ...
        div(toList([class$("flex items-center gap-3")]), toList([
          ...
          ]), toList([text2("Available for hire")]))
        ])),
...
// build/dev/javascript/client/pages/collection.mjs
var FILEPATH = "src/pages/collection.gleam";
function view2() {
  throw makeError("todo", FILEPATH, "pages/collection", 4, "view", "Collection page not yet implemented", {});
}

// build/dev/javascript/client/pages/home.mjs
function topic_card(emoji, label2) {
  return div(toList([
    class$("group relative bg-surface rounded-xl border border-border-light p-6 text-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer")
  ]), toList([
    div(toList([
      class$("text-3xl mb-3 group-hover:scale-110 transition-transform duration-300")
    ]), toList([text2(emoji)])),
    p(toList([class$("text-sm font-medium text-primary")]), toList([text2(label2)]))
  ]));
}
function view3(success3) {
  let _block;
  if (success3 instanceof Some) {
    let msg = success3[0];
    _block = div(toList([
      class$("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 animate-fade-in")
    ]), toList([
      div(toList([
        class$("p-4 bg-green-50 text-green-800 border border-green-200 rounded-lg text-sm font-medium")
      ]), toList([text2(msg)]))
    ]));
  } else {
    _block = none2();
  }
  let success_banner = _block;
  return div(toList([]), toList([
    success_banner,
    section(toList([
      class$("hero-bg min-h-[540px] flex items-center justify-center px-4")
    ]), toList([
      div(toList([
        class$("relative z-10 text-center max-w-3xl mx-auto animate-fade-in")
      ]), toList([
        h1(toList([
          class$("text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-6 leading-tight")
        ]), toList([text2("funsplash")])),
        p(toList([
          class$("text-lg sm:text-xl md:text-2xl text-gray-200 mb-4")
        ]), toList([
          text2("The internet's source of freely-usable images.")
        ])),
        p(toList([class$("text-sm text-gray-400 mb-10")]), toList([text2("Powered by creators everywhere.")])),
        div(toList([class$("max-w-2xl mx-auto animate-slide-up")]), toList([
          div(toList([
            class$("flex items-center bg-white rounded-2xl overflow-hidden shadow-2xl")
          ]), toList([
            span(toList([class$("pl-5 text-gray-400 text-lg")]), toList([text2("\uD83D\uDD0D")])),
            input(toList([
              type_("text"),
              placeholder("Search free high-resolution photos"),
              class$("w-full px-4 py-4 text-gray-900 text-base sm:text-lg bg-transparent"),
              attribute2("readonly", "true")
            ]))
          ]))
        ]))
    ...
    section(toList([class$("bg-surface-dim py-20 px-4 text-center")]), toList([
      div(toList([class$("max-w-2xl mx-auto")]), toList([
        h2(toList([
          class$("text-3xl font-bold text-primary mb-4")
        ]), toList([text2("Join the community")])),
        p(toList([class$("text-accent mb-8 text-lg")]), toList([
          text2("Upload your photos and share them with the world.")
        ])),
        a(toList([
          href("/join"),
          class$("inline-block px-8 py-3 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover transition-colors text-lg")
        ]), toList([text2("Get started →")]))
      ]))
    ]))
  ]));
}

// build/dev/javascript/client/pages/not_found.mjs
function view4() {
  return div(toList([
    class$("min-h-[calc(100vh-12rem)] flex items-center justify-center px-4 py-16 animate-fade-in")
  ]), toList([
    div(toList([class$("text-center max-w-md")]), toList([
      div(toList([class$("text-8xl mb-8")]), toList([text2("\uD83D\uDD0D")])),
      h1(toList([class$("text-4xl font-bold text-primary mb-4")]), toList([text2("Page not found")])),
      p(toList([class$("text-accent mb-8 text-lg")]), toList([
        text2("The page you're looking for doesn't exist or has been moved.")
      ])),
      a(toList([
        href("/"),
        class$("inline-block px-8 py-3 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover transition-colors")
      ]), toList([text2("← Go home")]))
    ]))
  ]));
}

// build/dev/javascript/shared/shared/shared_routes.mjs
var photo_data = "photo-";
var photo_data_premium = "premium_photo-";

// build/dev/javascript/client/pages/photo.mjs
...
function loading_view() {
  return div(toList([
    class$("max-w-5xl mx-auto px-4 py-12 animate-pulse-soft")
  ]), toList([
    div(toList([class$("skeleton w-full h-96 mb-8")]), toList([])),
    div(toList([class$("skeleton w-1/2 h-6 mb-4")]), toList([])),
    div(toList([class$("skeleton w-1/3 h-4")]), toList([]))
  ]));
}
function error_view() {
  return div(toList([
    class$("max-w-2xl mx-auto px-4 py-24 text-center animate-fade-in")
  ]), toList([
    div(toList([class$("text-6xl mb-6")]), toList([text2("\uD83D\uDE15")])),
    h1(toList([class$("text-2xl font-bold text-primary mb-3")]), toList([text2("Photo not found")])),
    p(toList([class$("text-accent mb-8")]), toList([
      text2("This photo may have been removed or doesn't exist.")
    ])),
    a(toList([
      href("/"),
      class$("inline-block px-6 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium")
    ]), toList([text2("← Back to home")]))
  ]));
}
function stat_row(icon, label2, value2) {
  return div(toList([class$("flex items-center justify-between")]), toList([
    span(toList([
      class$("flex items-center gap-2 text-sm text-accent")
    ]), toList([
      span(toList([]), toList([text2(icon)])),
      text2(label2)
    ])),
    span(toList([class$("text-sm font-medium text-primary")]), toList([text2(value2)]))
  ]));
}
function photo_image_url(thumb) {
  let $ = thumb.premium;
  if ($) {
    return "/" + photo_data_premium + thumb.asset_id;
  } else {
    return "/" + photo_data + thumb.asset_id;
  }
}
function photo_view(photo) {
  let thumb = photo.thumbnail;
  let stats = photo.stats;
  let image_url = photo_image_url(thumb);
  return div(toList([class$("max-w-5xl mx-auto px-4 py-8 animate-fade-in")]), toList([
    div(toList([class$("flex items-center justify-between mb-6")]), toList([
      a(toList([
        ...
        div(toList([
          class$("w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-lg font-bold text-accent")
        ]), toList([text2("\uD83D\uDC64")])),
        div(toList([]), toList([
          p(toList([
            class$("text-sm font-medium text-primary group-hover:underline")
          ]), toList([text2(thumb.creator)])),
          (() => {
            let $ = thumb.premium;
            if ($) {
              return span(toList([
                class$("text-xs text-premium font-medium")
              ]), toList([text2("★ Premium")]));
            } else {
              return none2();
            }
          })()
      ...
      div(toList([class$("flex items-center gap-3")]), toList([
        div(toList([
          class$("flex items-center gap-1.5 px-4 py-2 border border-border rounded-lg text-sm text-accent")
        ]), toList([
          span(toList([]), toList([
            text2((() => {
              let $ = thumb.user_liked;
              if ($) {
                return "❤️";
              } else {
                return "\uD83E\uDD0D";
          ...
          text2(to_string(stats.likes))
        ])),
        a(toList([
          href(image_url),
          attribute2("download", ""),
          class$("flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium")
        ]), toList([
    ...
    div(toList([
      class$("rounded-xl overflow-hidden bg-gray-100 mb-8")
    ]), toList([
      img(toList([
        src(image_url),
        alt((() => {
          let _pipe = thumb.description;
          return unwrap(_pipe, "Photo on funsplash");
        })()),
        class$("w-full h-auto max-h-[75vh] object-contain mx-auto")
      ]))
    ])),
    div(toList([class$("grid grid-cols-1 md:grid-cols-3 gap-8")]), toList([
      div(toList([class$("md:col-span-2 space-y-4")]), toList([
        (() => {
          let $ = photo.title;
          if ($ instanceof Some) {
            let title = $[0];
        ...
        (() => {
          let $ = photo.thumbnail.description;
          if ($ instanceof Some) {
            let desc = $[0];
            return p(toList([class$("text-accent leading-relaxed")]), toList([text2(desc)]));
          } else {
            return none2();
          }
        })(),
        (() => {
          let $ = is_empty2(photo.tags);
          if ($) {
            return none2();
          } else {
            return div(toList([class$("flex flex-wrap gap-2 pt-2")]), map2(photo.tags, (tag) => {
              return span(toList([
                class$("px-3 py-1 bg-surface-dim text-sm text-accent rounded-md border border-border-light")
              ]), toList([text2(tag)]));
            }));
          }
        })()
      ...
      div(toList([
        class$("space-y-4 p-6 bg-surface-dim rounded-xl border border-border-light")
      ]), toList([
        stat_row("\uD83D\uDC41", "Views", to_string(stats.views)),
        stat_row("❤️", "Likes", to_string(stats.likes)),
        stat_row("⬇", "Downloads", to_string(stats.downloads)),
        (() => {
          let $ = photo.location;
          if ($ instanceof Some) {
            let loc = $[0];
            return stat_row("\uD83D\uDCCD", "Location", loc);
          } else {
            return none2();
          }
        })(),
...
function view5(model) {
  return div(toList([class$("page-enter")]), toList([
    (() => {
...
class NotLoaded2 extends CustomType {
}
class Loading2 extends CustomType {
}
class Loaded2 extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class Error4 extends CustomType {
}
function loading_view2() {
  return div(toList([
    class$("max-w-5xl mx-auto px-4 py-12 animate-pulse-soft")
  ]), toList([
    div(toList([class$("flex items-center gap-6 mb-12")]), toList([
      div(toList([class$("skeleton w-24 h-24 rounded-full")]), toList([])),
      div(toList([class$("space-y-3")]), toList([
        div(toList([class$("skeleton w-48 h-6")]), toList([])),
        div(toList([class$("skeleton w-32 h-4")]), toList([]))
      ]))
    ])),
    div(toList([class$("grid grid-cols-3 gap-4")]), toList([
      div(toList([class$("skeleton h-48")]), toList([])),
      div(toList([class$("skeleton h-56")]), toList([])),
      div(toList([class$("skeleton h-44")]), toList([]))
    ]))
  ]));
}
function error_view2() {
  return div(toList([
    class$("max-w-2xl mx-auto px-4 py-24 text-center animate-fade-in")
  ]), toList([
    div(toList([class$("text-6xl mb-6")]), toList([text2("\uD83D\uDC64")])),
    h1(toList([class$("text-2xl font-bold text-primary mb-3")]), toList([text2("User not found")])),
    p(toList([class$("text-accent mb-8")]), toList([
      text2("This profile doesn't exist or has been removed.")
    ])),
    a(toList([
      href("/"),
      class$("inline-block px-6 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium")
...
function photo_card(thumb) {
  let _block;
  let $ = thumb.premium;
  if ($) {
    _block = "/" + photo_data_premium + thumb.asset_id;
  } else {
    _block = "/" + photo_data + thumb.asset_id;
  }
  let image_url = _block;
  return a(toList([
    href("/photos/" + thumb.public_id),
    class$("photo-card block")
  ]), toList([
    img(toList([
      src(image_url),
      alt((() => {
        let _pipe = thumb.description;
        return unwrap(_pipe, "Photo by " + thumb.creator);
      })()),
      class$("w-full rounded-lg"),
      attribute2("loading", "lazy")
    ])),
    div(toList([class$("photo-overlay")]), toList([
      div(toList([
        class$("w-full flex items-center justify-between")
      ]), toList([
        span(toList([class$("text-white text-sm font-medium")]), toList([text2(thumb.creator)])),
        div(toList([class$("flex items-center gap-2")]), toList([
          (() => {
            let $1 = thumb.premium;
            if ($1) {
              return span(toList([
                class$("text-premium text-xs font-bold")
              ]), toList([text2("★")]));
            } else {
              return none2();
            }
          ...
          (() => {
            let $1 = thumb.private;
            if ($1) {
              return span(toList([class$("text-white/70 text-xs")]), toList([text2("\uD83D\uDD12")]));
            } else {
              return none2();
            }
          })(),
          (() => {
            let $1 = thumb.user_liked;
            if ($1) {
              return span(toList([class$("text-red-400")]), toList([text2("❤️")]));
            } else {
              return none2();
            }
          })()
...
function photo_grid(photos) {
  return div(toList([class$("photo-grid")]), map2(photos, photo_card));
}
function empty_photos(is_own) {
  return div(toList([class$("text-center py-20")]), toList([
    div(toList([class$("text-5xl mb-4")]), toList([text2("\uD83D\uDCF7")])),
    p(toList([class$("text-lg text-accent mb-4")]), toList([
      text2((() => {
        if (is_own) {
          return "You haven't uploaded any photos yet";
        } else {
    ...
    (() => {
      if (is_own) {
        return a(toList([
          href2(new Upload),
          class$("inline-block px-6 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium")
        ]), toList([text2("Upload your first photo →")]));
      } else {
        return none2();
      }
    })()
...
function profile_view(user, auth_user) {
  let _block;
  let $ = user.last_name;
  if ($ instanceof Some) {
    let last = $[0];
    _block = user.first_name + " " + last;
  } else {
    _block = user.first_name;
  }
  let display_name = _block;
  let is_own = isEqual(auth_user, new Some(user.username));
  return div(toList([class$("animate-fade-in")]), toList([
    div(toList([
      class$("bg-surface-dim border-b border-border-light")
    ]), toList([
      div(toList([
        class$("max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col sm:flex-row items-center sm:items-start gap-6")
      ]), toList([
        div(toList([
          class$("w-28 h-28 rounded-full bg-gray-200 flex items-center justify-center text-5xl flex-shrink-0 ring-4 ring-white shadow-lg")
        ]), toList([text2("\uD83D\uDC64")])),
        div(toList([class$("text-center sm:text-left flex-1")]), toList([
          div(toList([
            class$("flex items-center gap-3 mb-1 justify-center sm:justify-start flex-wrap")
          ]), toList([
            h1(toList([
              class$("text-2xl font-bold text-primary")
            ]), toList([text2(display_name)])),
            (() => {
              let $1 = user.premium;
              if ($1) {
                return span(toList([
                  class$("px-2 py-0.5 text-xs font-bold text-premium bg-premium/10 rounded-full")
                ]), toList([text2("★ PREMIUM")]));
              } else {
                return none2();
              }
            ...
            (() => {
              if (is_own) {
                return span(toList([
                  class$("px-2 py-0.5 text-xs font-medium text-blue-700 bg-blue-100 rounded-full")
                ]), toList([text2("You")]));
              } else {
                return none2();
              }
          ...
          p(toList([class$("text-sm text-accent mb-3")]), toList([text2("@" + user.username)])),
          (() => {
            let $1 = user.bio;
            if ($1 instanceof Some) {
              let bio = $1[0];
              return p(toList([
                class$("text-sm text-accent leading-relaxed max-w-lg")
              ]), toList([text2(bio)]));
            } else {
              return none2();
            }
          ...
          div(toList([
            class$("flex items-center gap-6 mt-4 justify-center sm:justify-start")
          ]), toList([
            div(toList([]), toList([
              span(toList([
                ...
                text2((() => {
                  let _pipe = length(user.photos);
                  return to_string(_pipe);
                })())
              ])),
              span(toList([
                class$("text-sm text-accent ml-1")
              ]), toList([text2("photos")]))
            ]))
          ])),
          (() => {
            if (is_own) {
              return div(toList([
                class$("flex items-center gap-3 mt-5 justify-center sm:justify-start")
              ]), toList([
                a(toList([
                  href2(new Upload),
                  class$("inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-hover transition-colors")
                ]), toList([
                  span(toList([]), toList([text2("⬆")])),
                  text2("Upload a photo")
                ])),
                a(toList([
                  href("/logout"),
                  class$("inline-flex items-center gap-2 px-5 py-2.5 border border-border text-sm font-medium text-accent rounded-lg hover:text-primary hover:border-primary transition-colors")
                ]), toList([text2("Log out")]))
              ]));
            } else {
              return none2();
      ...
      (() => {
        if (is_own) {
          return div(toList([
            ...
            ]), toList([text2("Your photos")])),
            a(toList([
              href2(new Upload),
              class$("text-sm text-accent hover:text-primary transition-colors")
            ]), toList([text2("+ Upload new")]))
          ]));
        } else {
          return h2(toList([
            class$("text-lg font-semibold text-primary mb-6")
          ]), toList([text2("Photos")]));
        }
      })(),
      (() => {
        let $1 = is_empty2(user.photos);
        if ($1) {
          return empty_photos(is_own);
        } else {
          return photo_grid(user.photos);
        }
      })()
    ]))
  ]));
}
function view6(model, auth_user) {
  return div(toList([class$("page-enter")]), toList([
    (() => {
      if (model instanceof NotLoaded2) {
        return loading_view2();
      } else if (model instanceof Loading2) {
        return loading_view2();
      } else if (model instanceof Loaded2) {
        let user = model[0];
        return profile_view(user, auth_user);
      } else {
        return error_view2();
      }
    })()
...
// build/dev/javascript/client/pages/upload.mjs
function field3(name2, label2, type_2, placeholder2) {
  return div(toList([]), toList([
    label(toList([
      for$(name2),
      class$("block text-sm font-medium text-primary mb-1.5")
...
function checkbox(name2, label2, checked2) {
  return div(toList([class$("flex items-center gap-3")]), toList([
    input(toList([
      type_("checkbox"),
      name(name2),
    ...
    label(toList([for$(name2), class$("text-sm text-accent")]), toList([text2(label2)]))
  ]));
}
function alert_banner2(message2) {
  return div(toList([
    class$("p-4 rounded-lg border text-sm animate-fade-in bg-red-50 text-red-800 border-red-200")
  ]), toList([text2(message2)]));
}
function view7(error) {
  let _block;
  if (error instanceof Some) {
    let err = error[0];
    _block = alert_banner2(err);
  } else {
    _block = none2();
  }
  let alert = _block;
  return div(toList([
    ...
    div(toList([
      class$("w-full max-w-2xl glass-card rounded-2xl p-8 animate-fade-in")
    ]), toList([
      div(toList([class$("text-center mb-8")]), toList([
        div(toList([class$("text-4xl mb-4")]), toList([text2("⬆️")])),
        h1(toList([
          class$("text-2xl font-bold text-primary mb-2")
        ]), toList([text2("Upload a photo")])),
        p(toList([class$("text-sm text-accent")]), toList([
          text2("Share your best work with the community.")
        ]))
      ])),
      div(toList([class$("space-y-4")]), toList([
        alert,
        form(toList([
          method("POST"),
          action("/upload"),
          attribute2("enctype", "multipart/form-data"),
          class$("space-y-5")
        ]), toList([
          div(toList([]), toList([
            label(toList([
            ...
            input(toList([
              ...
              name("photo"),
              id("photo"),
              accept(toList(["image/png", "image/jpeg", "image/webp"])),
              attribute2("required", "true"),
              class$("w-full px-4 py-3 border border-border rounded-lg text-sm text-primary bg-white file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-primary file:text-white hover:file:bg-primary-hover file:cursor-pointer file:transition-colors")
            ]))
          ])),
          div(toList([]), toList([
            label(toList([
          ...
          div(toList([
            ...
            field3("location", "Location", "text", "Where was this taken?"),
            field3("camera", "Camera", "text", "Camera model")
          ])),
          div(toList([]), toList([
            label(toList([
              for$("tags"),
            ...
            p(toList([class$("text-xs text-accent mt-1")]), toList([text2("Separate tags with commas")]))
          ])),
          div(toList([class$("space-y-3 pt-2")]), toList([
...
class Model extends CustomType {
  constructor(route, auth_user, photo_page, profile_page, error, success3) {
    super();
    this.route = route;
...
class GotMe extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class GotPhoto extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class GotProfile extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function fetch_me() {
  let decoder = field("username", string2, (username) => {
    return success(username);
  });
  let handler = expect_json(decoder, (var0) => {
    return new GotMe(var0);
  });
  return get3("/me", handler);
}
function fetch_for_route(route) {
  if (route instanceof PhotoById) {
    let id2 = route.id;
    let handler = expect_json(photo_decoder(), (var0) => {
      return new GotPhoto(var0);
    });
    return get3("/photos/" + id2, handler);
  } else if (route instanceof UserByName) {
    let name2 = route.name;
    let handler = expect_json(user_decoder(), (var0) => {
      return new GotProfile(var0);
    });
    return get3("/@" + name2, handler);
  } else {
    return none();
  }
}
function update2(model, msg) {
  if (msg instanceof OnRouteChange) {
    ...
    let _block;
    if (route instanceof PhotoById) {
      _block = new Model(route, model.auth_user, new Loading, model.profile_page, new None, new None);
    } else if (route instanceof UserByName) {
      _block = new Model(route, model.auth_user, model.photo_page, new Loading2, new None, new None);
    } else {
      _block = new Model(route, model.auth_user, model.photo_page, model.profile_page, new None, new None);
    }
    let model$1 = _block;
    return [model$1, fetch_for_route(route)];
  } else if (msg instanceof GotMe) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let username = $[0];
      return [
        new Model(model.route, new Some(username), model.photo_page, model.profile_page, model.error, model.success),
        none()
      ];
    } else {
      return [
        new Model(model.route, new None, model.photo_page, model.profile_page, model.error, model.success),
        none()
      ];
    }
  } else if (msg instanceof GotPhoto) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let p2 = $[0];
      return [
        new Model(model.route, model.auth_user, new Loaded(p2), model.profile_page, model.error, model.success),
        none()
      ];
    } else {
      return [
        new Model(model.route, model.auth_user, new Error3, model.profile_page, model.error, model.success),
        none()
      ];
    }
  } else {
    ...
    if ($ instanceof Ok) {
      let user = $[0];
      return [
        new Model(model.route, model.auth_user, model.photo_page, new Loaded2(user), model.error, model.success),
        none()
      ];
    } else {
      return [
        new Model(model.route, model.auth_user, model.photo_page, new Error4, model.error, model.success),
        none()
      ];
    }
  }
...
function page_view(model) {
  ...
  if ($ instanceof Index2) {
    return view3(model.success);
  } else if ($ instanceof PhotoById) {
    return view5(model.photo_page);
  } else if ($ instanceof CollectionById) {
    return view2();
  } else if ($ instanceof UserByName) {
    return view6(model.profile_page, model.auth_user);
  } else if ($ instanceof Login) {
    return login_view(model.error, model.success);
  } else if ($ instanceof Join) {
    return signup_view(model.error);
  } else if ($ instanceof Upload) {
    return view7(model.error);
  } else {
    return view4();
  }
}
function view8(model) {
  return div(toList([
    class$("min-h-screen bg-white text-gray-900 flex flex-col font-sans")
  ]), toList([
    navbar(model.route, model.auth_user),
    main(toList([class$("flex-1")]), toList([page_view(model)])),
    footer2()
  ]));
}
function get_query_param(initial_uri_result, key) {
  if (initial_uri_result instanceof Ok) {
    let uri = initial_uri_result[0];
    let $ = uri.query;
    if ($ instanceof Some) {
      let q = $[0];
      let $1 = parse_query(q);
...
function init2(_) {
  let initial_uri = do_initial_uri();
  let _block;
  let _pipe = initial_uri;
  let _pipe$1 = map4(_pipe, parse_route);
  _block = unwrap2(_pipe$1, new Index2);
  let route = _block;
  let error = get_query_param(initial_uri, "error");
  let success3 = get_query_param(initial_uri, "success");
  let model = new Model(route, new None, new NotLoaded, new NotLoaded2, error, success3);
  let effects = batch(toList([
    init((uri) => {
      return new OnRouteChange(parse_route(uri));
    }),
    fetch_me(),
...
function main2() {
  let app = application(init2, update2, view8);
  let $ = start4(app, "#app", toList([]));
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH2, "client", 27, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 591, end: 639, pattern_start: 602, pattern_end: 607 });
  }
  return;
}
