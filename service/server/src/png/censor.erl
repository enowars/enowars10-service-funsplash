...
process_rows(PhotoRowBytes, MaskRowBytes, BitDepth, ColorType, RawPhoto, Mask, Acc) ->
    ...
    case is_black_censored(MaskRow) of
		true -> 
            [[0, <<0:(PhotoRowBytes*8)>>] | process_rows(PhotoRowBytes, MaskRowBytes, BitDepth, ColorType, PhotoRest, MaskRest, Acc)];
		false ->
            case is_transparent_censored(MaskRow) of
                true ->
                    [[0, RawRow] | process_rows(PhotoRowBytes, MaskRowBytes, BitDepth, ColorType, PhotoRest, MaskRest, Acc)];
                false ->
                    NewRow = if
...
is_black_censored(<<0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255,
                    0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, Rest/binary>>) ->
    is_black_censored(Rest);
is_black_censored(<<0, 0, 0, 255, Rest/binary>>) ->
    is_black_censored(Rest);
is_black_censored(<<>>) -> true;
is_black_censored(_) -> false.
...
merge_gray(<<P:8/binary, PRest/binary>>,
           <<_:24, 0:8, _:24, 0:8, _:24, 0:8, _:24, 0:8,
             _:24, 0:8, _:24, 0:8, _:24, 0:8, _:24, 0:8, MRest/binary>>, Acc) ->
    merge_gray(PRest, MRest, <<Acc/binary, P/binary>>);
merge_gray(<<_:8/binary, PRest/binary>>,
           <<_:24, 255:8, _:24, 255:8, _:24, 255:8, _:24, 255:8,
             _:24, 255:8, _:24, 255:8, _:24, 255:8, _:24, 255:8, MRest/binary>>, Acc) ->
    merge_gray(PRest, MRest, <<Acc/binary, 0:64>>);
