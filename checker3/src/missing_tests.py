...
async def putnoise_update_delete(
    ...
    r = await conn.post(
        f"/napi/collections/{col_id}",
        data={"name": col_name + "updated", "description": "desc", "private": "false"},
        cookies=cookies,
    ...
    r = await conn.delete(
        f"/napi/collections/{col_del_id}", cookies=cookies, follow_redirects=False
    )
    ...
    r = await conn.delete(
        f"/napi/photos/{pid_del}", cookies=cookies, follow_redirects=False
    )
...
async def getnoise_update_delete(
    ...
    assert_equals(
        photo_obj.public_id, pid, "Photo description was not updated correctly"
    )
...
async def putnoise_show_on_profile(
    ...
    p = photo.Photo(
        description=hidden_desc, data=utils.placeholder_png(), show_on_profile=False
    )
    await photo.upload(conn, cookies, p)
    await db.set("user", asdict(u))
    ...
    for p in photos:
        if hidden_desc in p.get("description", ""):
            raise MumbleException(
                "show_on_profile=False photo is visible to anonymous users"
            )
...
async def putnoise_login_logout(
    ...
    if r.status_code not in (401, 403, 404, 303):
        raise MumbleException(
            f"Logout failed, session still active. Got status: {r.status_code}"
        )
...
async def putnoise_censor_visual(
    ...
    p = photo.Photo(
        description=desc, privacy=Privacy.Premium, data=utils.placeholder_png()
    )
    await photo.upload(conn, cookies, p)
...
async def getnoise_censor_visual(
    ...
    profile = await user.get_profile(conn, u.name, cookies)
    censored_photo_obj = photo.get_by_description_contains(
        profile, desc + " (censored)"
    )

    # Now retrieve the image bytes
    censored_data = await photo.get_data_premium(
        conn, censored_photo_obj.asset_id, cookies=cookies
    )

    if censored_data == orig_data:
        raise MumbleException(
            "Censoring failed, the retrieved image is identical to the original"
        )
...
async def putnoise_collection_photos(
    ...
    p2 = photo.Photo(
        description="collection photo remove", data=utils.placeholder_png()
    )
    await photo.upload(conn, cookies, p2)
    profile = await user.get_profile(conn, u.name, cookies)
    pid2 = photo.get_by_description_contains(
        profile, "collection photo remove"
    ).public_id
...
async def getnoise_tags_update(
    ...
    for tag in expected_tags:
        if tag not in actual_tags:
            raise MumbleException(
                f"Tag {tag} was not found in updated photo. Tags found: {actual_tags}"
            )
...
async def putnoise_private_photo(
    ...
    r = await conn.get(
        f"/images/private_photo-{photo_obj.asset_id}", follow_redirects=False
    )
...
async def getnoise_private_photo(
    ...
    r = await conn.get(
        f"/images/private_photo-{asset_id}", cookies=cookies, follow_redirects=False
    )
    assert_equals(r.status_code, 200, "Creator should be able to fetch private photo")
    if not r.content.startswith(b"\x89PNG"):
        raise MumbleException(
            f"Private photo content mismatch! Expected PNG, got Start: {r.content[:20]}"
        )
...
async def getnoise_user_search(
    ...
    if not found:
        raise MumbleException(
            f"User search for {search_query} did not return {expected_username}"
        )
...
async def getnoise_update_collection(
    ...
    if col_data.get("name") != expected_name:
        raise MumbleException(
            f"Expected collection name {expected_name}, got {col_data.get('name')}"
        )
...
async def getnoise_update_photo(
    ...
    if p_obj.description != expected_desc:
        raise MumbleException(
            f"Expected photo description {expected_desc}, got {p_obj.description}"
        )
